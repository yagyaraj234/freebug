...
const fixture = async () => {
  const app = new Hono()
  app.get('/', (c) => c.html(`<!doctype html><html lang="en"><head><title>Fixture</title></head><body><main><h1>Welcome</h1><button id="start">Start</button><section id="dashboard" hidden><h2>Dashboard</h2></section></main><script>document.querySelector('#start').addEventListener('click',()=>document.querySelector('#dashboard').hidden=false)</script></body></html>`))
  return await new Promise<string>((resolve) => {
    const server = serve({ fetch: app.fetch, port: 0 }, (info) => resolve(`http://127.0.0.1:${info.port}`))
    servers.push(server)
  })
}
...
class FixedPlanner implements Planner {
  async plan(_run: Run): Promise<TestPlan> {
    return { summary: 'Exercise the primary user flow', tests: [{ id: 'primary-flow', title: 'Open dashboard', steps: [{ action: 'goto', path: '/' }, { action: 'assertText', text: 'Welcome' }, { action: 'click', role: 'button', name: 'Start' }, { action: 'assertText', text: 'Dashboard' }, { action: 'scanAccessibility' }] }] }
  }
}
class CapturingNotifier implements Notifier {
  runs: Run[] = []
  async sendCompleted(run: Run) { this.runs.push(run) }
}
...
describe('autonomous testing pipeline', () => {
  it('plans, executes, records evidence, reports, and notifies end to end', async () => {
    ...
    const artifactDir = await mkdtemp(join(tmpdir(), 'freebug-artifacts-')); directories.push(artifactDir)
    ...
    expect(run.results).toEqual([expect.objectContaining({ testId: 'primary-flow', status: 'passed' })])
    expect(run.artifacts?.map((artifact) => artifact.kind)).toEqual(expect.arrayContaining(['video', 'trace', 'report']))
