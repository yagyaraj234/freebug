...
export function generatePlaywrightScript(run: Run): string {
  ...
  const input = JSON.stringify({ targetUrl: run.targetUrl, tests: run.plan.tests })
...
const results = [], findings = [], artifacts = []
try {
  for (const test of input.tests) {
    ...
    try {
      ...
      artifacts.push({ kind: 'screenshot', path: screenshot })
    ...
    await context.close()
    artifacts.push({ kind: 'trace', path: trace })
    const videoPath = await video?.path()
    if (videoPath) artifacts.push({ kind: 'video', path: videoPath })
    results.push({ testId: test.id, status: error ? 'failed' : 'passed', durationMs: Date.now() - started, ...(error ? { error } : {}) })
  }
} finally {
  await browser.close()
}
await writeFile('results.json', JSON.stringify({ results, findings, artifacts }, null, 2))
`
}
